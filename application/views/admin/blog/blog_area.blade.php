<h3>{{ stripslashes($post->title) }}
	<small>
		<a 
			href="#!/blog_list" 
			id="back"
			onclick="shower('../admin/blog_list', 'back', 'work_area', false)"
			class="btn btn-small"
...
<? $json_save = '{"id": "'.$post->id.'", "title": "\'+encodeURI($(\'#title_'.$post->id.'\').val())+\'", "text": "\'+encodeURI($(\'#text_'.$post->id.'\').html())+\'", "access": "\'+$(\'#post_access_'.$post->id.'\').val()+\'", "tags": "\'+encodeURI($(\'#tags_'.$post->id.'\').val())+\'", "lang": "\'+$(\'#lang_'.$post->id.'\').val()+\'"}'; ?>
			...
			<button 
				...
				type="button"
				disabled="disabled" 
				onclick="showerp('<?= htmlspecialchars($json_save) ?>', '../admin/blog_area/save', 'save_button_{{ $post->id }}', 'status_{{ $post->id }}', false, true); ">
...
<script>
	function save_btn(){
		showerp('<?= $json_save ?>', '../admin/blog_area/save', 'save_button_{{ $post->id }}', 'status_{{ $post->id }}', false, true);
	}
</script>