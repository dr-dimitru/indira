<h3>{{ stripslashes($post->title) }}
	<small>
		<a 
			href="#!/posts_list" 
			id="back"
			onclick="shower('../admin/posts_list', 'back', 'work_area', false)"
			class="btn btn-small"
	...
	<div class="span8">
		...
		<div class="btn-group">
			<button 
				...
				class="btn"
				type="button" 
				onclick="$('#text_{{ $post->id }}').redactor(); $('#edit_button_{{ $post->id }}').attr('disabled', 'disabled'); $('#save_button_{{ $post->id }}, #text_{{ $post->id }}').attr('disabled', false);">
					<i class="icon-pencil" style="color:#5bc0de"></i> {{ Lang::line('content.edit_word')->get(Session::get('lang')) }}
			</button>
			<a href="{{ URL::to('/'.$post->id.'?edit=true') }}" class="btn btn-inverse"><i class="icon-lemon" style="color: rgb(255, 194, 0);"></i> {{ Lang::line('forms.edit_on_page')->get(Session::get('lang')) }}</a>

<? $json_save = '{"id": "'.$post->id.'", "title": "\'+encodeURI($(\'#title_'.$post->id.'\').val())+\'", "text": "\'+encodeURI($(\'#text_'.$post->id.'\').html())+\'", "access": "\'+$(\'#post_access_'.$post->id.'\').val()+\'", "media": "\'+$(\'#selected_media_'.$post->id.'\').val()+\'", "section": "\'+$(\'#section_'.$post->id.'\').val()+\'", "tags": "\'+encodeURI($(\'#tags_'.$post->id.'\').val())+\'", "lang": "\'+$(\'#lang_'.$post->id.'\').val()+\'"}'; ?>
			...
			<button 
				...
				type="button"
				disabled="disabled" 
				onclick="showerp('<?= htmlspecialchars($json_save) ?>', '../admin/post_area/save', 'save_button_{{ $post->id }}', 'status_{{ $post->id }}', false, true); ">
...
<div class="row-fluid">
	<div class="span12">
		<div
			...
			id="text_{{ $post->id }}" 
			rows="20">{{ $post->text }}</div>
	</div>
</div>
<script>
	function save_btn(){
		showerp('<?= $json_save ?>', '../admin/post_area/save', 'save_button_{{ $post->id }}', 'status_{{ $post->id }}', false, true);
	}
</script>